...
from global_utils import config as config
    ...
    def __getitem__(self, index):
        ...
        if self.param_dir is not None:
            ...
            with np.load(os.path.join(self.param_dir, inp_path)) as f:
                item['pred_pose'] = f.f.pose.to(self.device)
                item['pred_shape'] = f.f.betas.to(self.device)
                item['pred_camera'] = f.f.camera.to(self.device)
                item['pred_trans'] = f.f.trans.to(self.device)
...
class AnimalDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # seg = seg_from_anno(a)  # (H, W) bool
        # seg = seg.astype(np.float) * 255.
        segname_raw = a['mask_path']
        segname = join(self.seg_dir, segname_raw)
        segname = segname.replace("\\", "/")
        assert os.path.exists(segname), "Cannot find mask: {0}".format(imgname)
        seg = cv2.imread(segname)[:,:,::-1].copy().astype(np.float32)*255
        seg = np.dstack([seg, seg, seg])  # (H, W, 3) as float

        x0, y0, width, height = a['bbox']
        
        scaleFactor = 1.2
        scale = scaleFactor*max(width, height)/200

        # kp_S24 = np.array(a['keypoint_2d'], dtype=np.float32)
        kp_S24 = np.array(a['reproj_kp_2d'], dtype=np.float32) + np.array([[100,-50,0]])
        center = np.array([x0+width/2, y0+height/2])  # Center of dog
