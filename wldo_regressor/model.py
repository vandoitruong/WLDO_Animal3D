...
class Model(nn.Module):
    def __init__( self, device, shape_family_id, load_from_disk, **kwargs):
        ...
        self.model_renderer = NeuralRenderer(
            ...
            norm_z=config.NORM_Z).to(self.device)
        ...
        self.netG_DETAIL = MeshNet(
            ...
            norm_f0=config.NORM_F0,
            nz_feat=config.NZ_FEAT).to(self.device)
    ...
    def forward(self, batch_input, demo=False):
        ...
        img = batch_input['img'].to(self.device)
        keypoints = batch_input['keypoints'].to(self.device)
        seg=batch_input['seg'].to(self.device)
        index=batch_input['index'].to(self.device)
        has_seg=batch_input['has_seg'].to(self.device)
        dataset=batch_input['dataset']
        img_orig=batch_input['img_orig'].to(self.device)
        scale=batch_input['scale'].to(self.device)
        img_border_mask=batch_input['img_border_mask'].to(self.device)
        ...
        if not self.load_from_disk:
            ...
            scale_pred, trans_pred, pose_pred, betas_pred, betas_logscale = \
                pred_codes
            # scale_pred = scale_pred.cuda()
            scale_pred = scale_pred.to(self.device)
            trans_pred = trans_pred.to(self.device)
            pose_pred = pose_pred.to(self.device)
            betas_pred = betas_pred.to(self.device)
            betas_logscale = betas_logscale.to(self.device)

            all_betas = torch.cat([betas_pred, betas_logscale], dim = 1).to(self.device)
            pred_camera = torch.cat([
                    scale_pred[:, [0]].to(self.device), 
            ...
            all_betas = batch_input['pred_shape'].to(self.device)
            pose_pred = batch_input['pred_pose'].reshape(-1, 105).to(self.device)
            trans_pred = batch_input['pred_trans'].to(self.device)
            pred_camera = batch_input['pred_camera'].to(self.device)
